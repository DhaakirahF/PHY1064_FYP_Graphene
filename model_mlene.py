...
def compare_SC_ML (x, y, subs, moms, mom_preds, dotsize=40, figsize=12):
    ...
    ax.plot([0,1], [0,1], '--')
    ax.set_ylim(0, 0.15)  #--------------change back to 0.4
    ax.set_ylabel('predicted moments', fontsize=15)
    ax.set_xlim(0, 0.15)#-------------------change back to 0.4
...
def compare_levels (x, y, subs, moms, mom_preds, emin, emax):
    '''Compare the energy levels of a flake, calculated using both ML and SC moments, and the nonmagnetic case'''
